...
- **Driver:** Imagick when the extension is loaded, else GD (`ImageManagerFactory`). Text metrics and rendering differ between drivers — don't assume pixel-exact parity. Critically, v4's **Imagick driver has no built-in font mechanism**: rendering text without a real font file throws `StateException: No font file specified` (GD falls back to its internal font). CI runners (setup-php) preinstall Imagick, so CI exercises the Imagick path while local dev typically exercises GD; text-rendering tests use `CanvasTestCase::systemTtf()` (skips when unavailable) and `usesImagickDriver()` to stay portable.
